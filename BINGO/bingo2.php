...
$fueradelBombo=array();
    ...
    while($numerosTachados < count($carton)) { //Aquí le digo: Oye, haz esto hasta que haya sacado los 15 números de mi cartón; es decir, hasta que haya 15/15 tachados. 
        ...
        while (in_array($numeroAleatorio,$fueradelBombo)) { //Para que no se repita, le digo: Si el numero aleatorio ya estaba fuera del bombo, no se puede volver a sacar! Porque no debería estar.

             $numeroAleatorio=rand(1,60); //Así que, si el aleatorio que ha salido coincide con uno que ya estaba fuera del bombo, entonces vuelve a generar otro. Y así hasta que no coincida con ninguno de los que estaban ya fuera. 
        }

            array_push($fueradelBombo,$numeroAleatorio); //Y cuando saque esa bola, hay que añadirla a esa caja "fuera del bombo".
    ...
    echo "Has conseguido el BINGO. Enhorabuena, Sergio :)"; //joder, ha costado. ¡Poco a poco! 
