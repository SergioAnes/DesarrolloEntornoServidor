...
function comprobarDNIRegistrado2($dniCliente,$base,$contra) {

	$sqlConsulta2="SELECT nif FROM cliente WHERE nif='$dniCliente' AND clave='$contra'"; 
	$resultado2 = $base-> query($sqlConsulta2);
	$dniCorrecto;
	if($resultado2->rowCount() == 1) { //Se supone que cuenta si el resultado de la consulta está en alguna de las filas. Si está es true (1).
		$dniCorrecto=true;
	}else{
		$dniCorrecto=false;
	}
...
//EJERCICIO 12
function obtenerNombreProducto ($idProducto, $base) {

	$sqlConsulta = "SELECT producto.nombre as nombre FROM producto WHERE id_Producto='$idProducto'";
	$resultado = $base-> prepare($sqlConsulta);
	$resultado->execute();
	while ($registro = $resultado->fetch(PDO::FETCH_ASSOC)) { 
...
}//fin obtener NombreProducto


function mostrarProductosCesta ($carroCompra, $base) {
	$longitudCarro = count($carroCompra); //Se cuenta la longitud del carro de la compra. 

	if ($longitudCarro==0) { //Si el carro tiene una longitud 0, es porque no tiene nada. 
		echo "El carro de la compra está vacío!";
	} else {
	//ahora habría que recorrer el carro según el producto. 
		for ($i=0; $i<$longitudCarro; $i++) {
			echo "Artículo " . ($i+1) . " " . obtenerNombreProducto($carroCompra[$i], $base) . "<br>"; //$carroCompra[$i] contiene la id de un product, y con esa id se obtiene el nombre.
		}//fin bucle for
	}//fin else 
}//fin mostrarProductosCesta 


function mostrarProductosCesta2 ($carroCompra, $base) {
	$longitudCarro = count($carroCompra); //Se cuenta la longitud del carro de la compra. 
	...
	//ahora habría que recorrer el carro según el producto. 
		...
		foreach ($carroCompra as $idProducto => $cantidad) {
				echo "Artículo " . ($i+1) . " " . obtenerNombreProducto($idProducto, $base) . " Unidades: " . $cantidad . "<br>";
				$i++; //$carroCompra[$i] contiene la id de un product, y con esa id se obtiene el nombre.
		} //fin foreach
	}//fin else 
}//fin mostrarProductosCesta 

function comprobarStockProducto2($carroCompra, $unidadesAComprar,$base){
...
function insertarCompras2 ($nif, $carroCompra, $base) {
	foreach ($carroCompra as $idProducto => $cantidad){
    //Insercion en la tabla almacena de los registros: 
    		$now=date("Y/m/d");
			$sqlConsulta = "INSERT INTO compra VALUES ('$nif', '$idProducto', '$now', '$cantidad')"; 
	 	try {
			$resultado = $base-> prepare($sqlConsulta); // Se prepara.
			$resultado->execute(); //Se ejecuta. 
		} catch(PDOException $e){
...
function actualizarStock2 ($base, $carroCompra) { //Lo único que si hay dos almacenas resta el id del producto a los dos, no a uno de ellos. 
	...
	/*A qué almacen se lo quito? */
foreach ($carroCompra as $idProducto => $cantidad){
	$sqlConsulta = "UPDATE almacena SET cantidad = cantidad - '$cantidad' WHERE id_producto = '$idProducto'"; 
	 try {
		$resultado = $base-> prepare($sqlConsulta); // Se prepara.
		$resultado->execute(); //Se ejecuta. 
		echo "Actualización correcta";
